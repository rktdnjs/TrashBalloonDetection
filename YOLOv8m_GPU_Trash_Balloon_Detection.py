...
model = YOLO('yolo_Detection/models/YOLOv8m_best_trash_balloon.pt')  # 커스텀 모델 로드
...
captured_num = 0
last_capture_time = 0
CAPTURE_INTERVAL = 30  # 5초 간격으로 캡쳐

def detectAndDisplay(frame):
    global captured_num, last_capture_time
    current_time = time.time()

    # OpenCV는 기본적으로 BGR로 이미지를 읽음 -> YOLOv8 모델은 RGB 형식을 기대
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    # YOLOv8 추론 수행
    results = model(frame_rgb)  # YOLOv8 프레임워크에서 추론
    ...
    cv2.imshow("YOLOv8m Trash Balloon Detection", resized_frame)
