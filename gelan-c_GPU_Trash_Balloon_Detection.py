...
# gelan-c 모델 로드 (torch.hub.load 사용)
# model_path = 'yolo_Detection/models/gelan_c_best_trash_balloon.pt'
# model = torch.load('.', 'custom', path=model_path, source='local')
# model = torch.load('yolo_Detection/models/gelan_c_best_trash_balloon.pt')
model_path = 'yolo_Detection/models/gelan_c_best_trash_balloon.pt'
...
def detectAndDisplay(frame):
    ...
    # YOLOv8 추론 수행
    results = model(frame_rgb)  # gelan-c(YOLOv9) 프레임워크에서 추론
    ...
    for box in detections:
        ...
        if class_id == 0 and conf > 0.488:  # 클래스 ID 0는 '풍선'일 경우
            # 좌표를 정수형으로 변환하여 사용
            ...
            if len(detections) > 0 and current_time - last_capture_time >= CAPTURE_INTERVAL:
                ...
                img_path = f'yolo_Detection/detection_result_image/output_{file_timestamp}_{captured_num}.jpg'

                # 바운딩 박스가 그려진 이미지를 저장
                cv2.imwrite(img_path, frame)  # 'frame'을 저장

                # 이미지를 백엔드로 전송
                # send_data_to_backend(img_path, 35.1895, 126.9340, timestamp)

    # 화면 크기를 조정 (예: 640x480 크기로 변경)
    resized_frame = cv2.resize(frame, (1920, 1080))  # 원하는 크기로 조정

    # 바운딩박스가 그려진 프레임을 화면에 표시
    cv2.imshow("gelan-c(YOLOv9) Trash Balloon Detection", resized_frame)
